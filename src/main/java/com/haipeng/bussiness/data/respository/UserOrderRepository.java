...
public interface UserOrderRepository extends CrudRepository<UserOrder,Long>{
    ...
    @Query(value = "select o.* from user_order o where u.user_unique_number =?1",nativeQuery = true)
    List<UserOrder> getOrdersByUserUniqueNumnber(long uniqueNumber);

    @Query(value = "select o.* from user_order o where u.unique_number =?1",nativeQuery = true)
